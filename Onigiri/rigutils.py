...
def build_devkit_rig(target="female_neutral", connect=True):
    ...
    rig_data = getattr(base_data, target)
    armObj['rig_class'] = target

    
    bpy.ops.object.mode_set(mode='EDIT')
    for bone in rig_data:
        if bone not in skel.avatar_skeleton:
            print("skipping goofy bone", bone)
            continue
        boneObj = armObj.data.edit_bones.new(bone)
        boneObj.matrix = rig_data[bone]['matrix_local']
    ...
    if connect == True:
        ...
        for boneObj in armObj.data.edit_bones:
            bone = boneObj.name
            boneObj.use_connect = rig_data[bone]['connected']
        bpy.ops.object.mode_set(mode='OBJECT')
...
def rebind(arm, keep_animation=False, report=False):

    print("rigutils::rebind reports - animation now travels with the rebind if (keep_animation=True), re-examine this for issues")
    ...
    if 1 == 0:
        for mod_name in bpy.context.preferences.addons.keys():
            ...
            if name == "Onigiri":
                ...
                break
        armObj['onigiri'] = utils.get_bento_buddy_version()
...
def build_rig(rig_class="pos", rotate=False, connect=False):
    ...
    if 1 == 0:
        print("Adjusting bone orientation specific for SL")
        bpy.ops.object.mode_set(mode='POSE')
        for boneObj in armObj.pose.bones:
            bone = boneObj.name
            deg = skel.avatar_skeleton[bone]['rot']
            rot = [math.radians(a) for a in deg] 
            R = mathutils.Euler(rot,'XYZ').to_matrix().to_4x4()
            m = boneObj.matrix.copy()
            M = get_matrix_basis(armature=armObj, bone=bone)
            
            boneObj.matrix_basis = R @ M
        bpy.ops.pose.armature_apply(selected=False)
        bpy.ops.object.mode_set(mode='OBJECT')


    print("Bone roll reduced to 0 for good rigs")
    bpy.ops.object.mode_set(mode='EDIT')
    for boneObj in armObj.data.edit_bones:
        boneObj.roll = 0.0
...
def get_mesh(arm):
    ...
    if len(mesh) > 0:
        return mesh
    print("rigutils::get_mesh : returning False")
    return False
...
def get_associated_mesh(arm, report=False):
    ...
    if isinstance(arm, str):
        if utils.is_valid(arm):
            ...
            if report == True:
                print("rigutils::get_associated_mesh reports : the armature is not in the scene", arm)
            return False
    ...
    if len(mesh) == 0:
        if report == True:
            print("rigutils::get_associated_mesh reports : no mesh remains after filtering to process")
        return False
...
def make_complete(
    ...
    print("===============================================")
    print("len(armObj.data.bones):", len(armObj.data.bones))
    print("===============================================")

    if len(bad_bones) == len(armObj.data.bones):
        print("rigsutils::make_complete : None of the bones in the rig matched, there's nothing to do")
...
def normalize(armObj):
    ...
    for boneObj in armObj.data.bones:
        bone = boneObj.name
        if bone.startswith("ik"):
            continue
        if bone in globals.goofy_bones:
            continue
...
def avastar_to_sl(armObj):
    ...
    for boneObj in armObj.data.bones:
        ...
        mbone = "m" + bone
        if mbone not in skel.avatar_skeleton:
            print("rigutils::avastar_to_sl : unknown bone:", bone)
            continue
        names[bone] = mbone
...
def cycle_rig(armature=None, frame_start=0, frame_end=0):
    armObj = armature
    if isinstance(armature, str):
        armObj = bpy.data.objects[armature]

    if abs(frame_start - frame_end) == 0:
        print("cycle_rig reports: no frames offered, gathering from rig or scene")
        frame_start, frame_end = animutils.get_frame_range(armObj)
    ...
    print("Quaternion interpolations are cartesian, Eulers are not.")
    print("Recycling rig to capture Eulers...")
    ...
    if len(bpy.context.selected_objects) == 0 or bpy.context.selected_objects[0].name == armObj.name:
        print("It appears that the recycle failed, could not import the saved animation rig:", file_path)
        return False

    newObj = bpy.context.selected_objects[0]

    
    newObj.matrix_world = armObj.matrix_world.copy()

    
    try:
        os.remove(file_path)
        print("Deleted temporary file:", bvh_file)
    except Exception as e:
        txt = traceback.format_exc()
        print(txt)
        print("I couldn't delete the temporary animation file:", file_path)

    
    for p in armObj.keys():
        print("Transferring property:", p)
        newObj[p] = armObj[p]
    for boneObj in armObj.pose.bones:
        bone = boneObj.name
        if bone not in newObj.data.bones:
            continue
        for p in armObj.pose.bones[bone].keys():
            newObj.pose.bones[bone][p] = armObj.pose.bones[bone][p]
        for p in armObj.data.bones[bone].keys():
            newObj.data.bones[bone][p] = armObj.data.bones[bone][p]
    ...
    bpy.ops.object.mode_set(mode='OBJECT')
    bpy.ops.object.mode_set(mode='EDIT')
    for boneObj in newObj.data.edit_bones:
        for p in edit_props:
...
def map_convert(armObj):
    ...
    temp_map = {}
    for boneObj in armObj.data.bones:
        if boneObj.name in qualified:
            bone = boneObj.name
            tname = utils.get_temp_name()
            if tname in armObj.data.bones:
                print("collision, attempting repair:", bone, tname)
                tname = tname + "_0RP1"
            boneObj.name = tname
            cname = boneObj.name
            temp_map[cname] = rename_map[bone]
    
    for tbone in temp_map:
        bone = temp_map[tbone]
        
        boneObj = armObj.data.bones[tbone]
        boneObj.name = bone
        
        if boneObj.name != bone:
            print("- Fatal bone naming error -")
            print("From", tbone, "to", bone, "resulted in", boneObj.name)
...
def set_deform_view(arm, action=None):
    armObj = arm
    if isinstance(arm, str):
        armObj = bpy.data.objects[arm]
    ...
    if action == "hide":
        ...
        if mesh == False:
            print("rigutils::set_deform_view : reports that (get_associated_mesh) failed, why am I calling this?")
        else:
            print("Associated mesh count:", len(mesh))
            for m in mesh:
                print(" -", m.name)
            ...
            for meshObj in mesh:
                for vg in meshObj.vertex_groups:
                    bone_list.add(vg.name)
            for boneObj in armObj.data.bones:
                
                if boneObj.name not in bone_list:
                    boneObj.hide = True
...
def get_bone_path(armature=None, bone=None, check=False):
